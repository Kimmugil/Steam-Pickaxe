"""
seed_ui_text.py — ui_text 탭에 기본 키를 일괄 세팅
이미 값이 있는 키는 건드리지 않음 (관리자 커스텀 값 보존).

실행: python backend/seed_ui_text.py
"""
import sys, os
sys.path.insert(0, os.path.dirname(__file__))

from sheets.master_sheet import get_spreadsheet

# ─────────────────────────────────────────────────────────────────────────────
# UiTextContext FALLBACK과 동기화된 전체 키-값 목록
# ─────────────────────────────────────────────────────────────────────────────
...
def run():
    ...
    print("ui_text 탭 시딩 시작...")
    ss = get_spreadsheet()

    # ui_text 탭 찾기 또는 생성
    existing_tabs = [ws.title for ws in ss.worksheets()]
    if "ui_text" not in existing_tabs:
        ws = ss.add_worksheet(title="ui_text", rows=500, cols=3)
        ws.append_row(["key", "value", "description"])
        print("[신규] ui_text 탭 생성 완료")
    else:
        ws = ss.worksheet("ui_text")
        print("[기존] ui_text 탭 발견")

    # 기존 키 읽기 (값이 있는 키만 보존 대상)
    ...
    if to_add:
        ...
        print(f"[완료] {len(to_add)}개 키 추가됨")
    else:
        print("[완료] 추가할 키 없음 (이미 모두 세팅됨)")

    print(f"[스킵] {len(existing)}개 키 보존됨 (값 있는 키)")
    print(f"\n총 {len(FALLBACK)}개 키 / 추가 {len(to_add)}개 / 보존 {len(existing)}개")
