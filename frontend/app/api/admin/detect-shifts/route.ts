...
/**
 * POST /api/admin/detect-shifts
 * detect-shifts.yml 워크플로우를 수동으로 트리거합니다.
 * 매주 월요일 자동 실행되지만, 즉시 실행이 필요할 때 사용합니다.
 *
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (appid) inputs.appid = String(appid);

    const res = await fetch(
      `https://api.github.com/repos/${GITHUB_REPO}/actions/workflows/detect-shifts.yml/dispatches`,
      {
        method: "POST",
        headers: {
